...
ENTRADAS = [
    ("principal", "data/metadata/cma_corpus_principal.csv"),
    ("secundario", "data/metadata/cma_corpus_secundario.csv"),
    ("descartados", "data/metadata/cma_descartados.csv"),
]
    ...
    for corpus_actual, ruta_relativa in ENTRADAS:
        ...
        for fila in filas:
            salida = {
                ...
                "motivo_revision": valor(fila, "motivo_auditoria"),
                "observaciones_revision": valor(fila, "observacion_auditoria"),
                "revisor": valor(fila, "revisor"),
                "fecha_revision": valor(fila, "fecha_revision"),
                "titulo_original": valor(fila, "titulo_original"),
                "titulo_es_sugerido": "",
                "cultura": valor(fila, "cultura") or valor(fila, "cultura_manual"),
                "fecha_objeto": valor(fila, "periodo") or valor(fila, "periodo_manual"),
                "procedencia": valor(fila, "cultura"),
                "tipo_objeto": valor(fila, "tipo_objeto_manual") or valor(fila, "nombre_objeto_original"),
                "tipo_superficie": "",
                "material": valor(fila, "material_original"),
                "material_normalizado": "",
                "tecnica": valor(fila, "tecnica_original"),
                "estado_curacion": valor(fila, "decision_auditoria"),
                "destino_curacion": corpus_actual,
                "motivo_curacion": valor(fila, "motivo_auditoria"),
                "url_objeto": valor(fila, "url_objeto"),
                "url_imagen": valor(fila, "url_imagen"),
                "ruta_imagen_local": "",
                "licencia": "",
                "origen_curacion": "revision_manual_cma",
            }

            if valor(fila, "revisor") == "auto":
                salida["origen_curacion"] = "descarte_automatico_cma"
...
def crear_hoja_revision(wb: Workbook, filas: list[dict[str, str]]) -> None:
    ws = wb.create_sheet("revision_cma_v2")
    ws.append(COLUMNAS)
...
def main() -> int:
    ...
    salida = raiz / "data/metadata/cma_revision_manual_v2.xlsx"
